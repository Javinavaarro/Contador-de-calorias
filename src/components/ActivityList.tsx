...
export default function ActivityList({activities, dispatch} : ActivityListProps) {
    ...
    const categoryName = useMemo(() => 
        (category: Activity['category']) => 
            categories.map(cat => cat.id === category ? cat.name : '') //Según el id obtenemos el name de esa categoria en la data bd
    ,[activities])
  ...
  return (
    ...
    <h2 className="text-4xl font-bold text-slate-600 text-center ">Comida y actividades</h2>
        ...
        {activities.length === 0 ? 
            <p className=" py-5 text-slate-600 text-center">Añade alguna comida o actividad</p>
            :
